...
def main(gpu_ids, img_height, img_width, model_name, model_path, stronger_levels_leave_early, eval_no_heads, eval_weighting, multiple_output, targets, train_file_path, 
																						queries_file_path, gallery_file_path, turbulance_dir_path, cfg):
	...
	if model_name == "TransReID":
		...
		model_online, model_momentum = getDCNN(gpu_indexes, model_name, stronger_levels_leave_early=stronger_levels_leave_early)
	...
	if targets == "MSMT17":
		balanced_accuracy = validate_with_valSet(train_images_target, val_images_target, img_height, img_width, model_online, 
		...
		validateOnDatasets(targets_names, [queries_images_target], [gallery_images_target], img_height, img_width, 
																			model_name, model_online, gpu_indexes, multipart=False)
	...
	if multiple_output:
		...
		meta_rec = Meta_Recognition()
		Nq, Ng = queries.shape[0], gallery.shape[0]

		queries_fvs_backbone, queries_fvs_head01, queries_fvs_head02 = extractFeatures(queries, img_height, img_width, model_online, 500, eval_no_heads, eval_weighting, multiple_output, gpu_indexes[0])
		gallery_fvs_backbone, gallery_fvs_head01, gallery_fvs_head02 = extractFeatures(gallery, img_height, img_width, model_online, 500, eval_no_heads, eval_weighting, multiple_output, gpu_indexes[0])

		weights_backbone = torch.maximum(torch.norm(queries_fvs_backbone, dim=1, keepdim=True).repeat(1, Ng), torch.norm(gallery_fvs_backbone, dim=1, keepdim=True).repeat(1, Nq).T)
		weights_head01 = torch.maximum(torch.norm(queries_fvs_head01, dim=1, keepdim=True).repeat(1, Ng), torch.norm(gallery_fvs_head01, dim=1, keepdim=True).repeat(1, Nq).T)
		weights_head02 = torch.maximum(torch.norm(queries_fvs_head02, dim=1, keepdim=True).repeat(1, Ng), torch.norm(gallery_fvs_head02, dim=1, keepdim=True).repeat(1, Nq).T)

		queries_fvs_backbone = queries_fvs_backbone/torch.norm(queries_fvs_backbone, dim=1, keepdim=True)
		gallery_fvs_backbone = gallery_fvs_backbone/torch.norm(gallery_fvs_backbone, dim=1, keepdim=True)

		queries_fvs_head01 = queries_fvs_head01/torch.norm(queries_fvs_head01, dim=1, keepdim=True)
		gallery_fvs_head01 = gallery_fvs_head01/torch.norm(gallery_fvs_head01, dim=1, keepdim=True)

		queries_fvs_head02 = queries_fvs_head02/torch.norm(queries_fvs_head02, dim=1, keepdim=True)
		gallery_fvs_head02 = gallery_fvs_head02/torch.norm(gallery_fvs_head02, dim=1, keepdim=True)

		distmat_backbone = 1.0 - torch.mm(queries_fvs_backbone, gallery_fvs_backbone.T)
		distmat_backbone = distmat_backbone.numpy()

		distmat_head01 = 1.0 - torch.mm(queries_fvs_head01, gallery_fvs_head01.T)
		distmat_head01 = distmat_head01.numpy()

		distmat_head02 = 1.0 - torch.mm(queries_fvs_head02, gallery_fvs_head02.T)
		distmat_head02 = distmat_head02.numpy()
		
		del queries_fvs_backbone, queries_fvs_head01, queries_fvs_head02
		del gallery_fvs_backbone, gallery_fvs_head01, gallery_fvs_head02

		calculate_metrics(distmat_backbone, queries, gallery)
		calculate_metrics(distmat_head01, queries, gallery)
		calculate_metrics(distmat_head02, queries, gallery)

		#distmat_ensemble = (weights_backbone*distmat_backbone + weights_head01*distmat_head01 + weights_head02*distmat_head02)/(weights_backbone + weights_head01 + weights_head02)
		#distmat_ensemble = 1.0 - meta_rec.mrfuse(1-torch.Tensor(distmat_backbone), 1-torch.Tensor(distmat_head01), 1-torch.Tensor(distmat_head02))
		distmat_ensemble = (distmat_backbone + distmat_head01 + distmat_head02)/3
		calculate_metrics(distmat_ensemble, queries, gallery)

	else:
		queries_fvs = extractFeatures(queries, img_height, img_width, model_online, 500, eval_no_heads, eval_weighting, multiple_output, gpu_indexes[0])
		gallery_fvs = extractFeatures(gallery, img_height, img_width, model_online, 500, eval_no_heads, eval_weighting, multiple_output, gpu_indexes[0])
		...
		calculate_metrics(distmat, queries, gallery)
...
def extractFeatures(subset, img_height, img_width, model, batch_size, eval_no_heads, eval_weighting, multiple_output, gpu_index=0, verbose=True):
    ...
    model.module.multiple_outputs = multiple_output
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            if multiple_output:
                fvs_backbone, fvs_head01, fvs_head02 = model(batch_gpu, eval_no_heads, False, False)
                ...
                fv = model(batch_gpu, eval_no_heads, False, False)
                fvs = fv.data.cpu()
...
if __name__ == '__main__':
	...
	main(gpu_ids, img_height, img_width, model_name, model_path, stronger_levels_leave_early, eval_no_heads, eval_weighting, multiple_output, 
